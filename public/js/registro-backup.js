...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Registro.js cargado e inicializado');
    ...
    console.log('Event listeners configurados, botones de contraseña encontrados:', botonesPassword.length);
});
...
function mostrarInfoEmpleado(empleado) {
    ...
    // Por ahora siempre puede crear credenciales (ya validamos en backend)
...
function configurarValidacionTiempoReal() {
    ...
    password.addEventListener('input', function() {
        ...
        if (confirmarPassword.value) {
            validarConfirmarPassword(password.value, confirmarPassword.value);
        }
    });
...
function validarCedula(cedula) {
    ...
    if (cedula.length < 7 || cedula.length > 10) {
        mostrarErrorCampo(campo, error, 'La cédula debe tener entre 7 y 10 dígitos');
        return false;
    }
...
function validarNombreUsuario(nombreUsuario) {
    ...
    if (!regex.test(nombreUsuario)) {
        mostrarErrorCampo(campo, error, 'Solo letras, números y guiones bajos. Mínimo 4 caracteres');
        return false;
    }
...
function validarPassword(password) {
    ...
    // Validación más simple: solo longitud mínima
...
async function manejarEnvioFormulario(event) {
    console.log('manejarEnvioFormulario llamado');
    ...
    if (!empleadoVerificado) {
        console.log('Empleado no verificado');
        mostrarMensaje('Primero debe verificar que es un empleado registrado', 'error');
        return;
    }
    
    console.log('Empleado verificado:', empleadoVerificado);
...
function togglePassword(fieldId) {
    console.log('togglePassword llamado con:', fieldId);
    ...
    console.log('Tipo actual del campo:', field.type);
    ...
    if (field.type === 'password') {
        ...
        `;
        console.log('Cambiado a visible');
    } else {
        field.type = 'password';
        button.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
...
function mostrarMensaje(mensaje, tipo) {
    ...
    mensajeElemento.querySelector('span').textContent = mensaje;
